...
class algorithm:
    ...
    def outside_d_square(self, i, j):
        ...
        random_query_list = self.random_query_list(2/self.epsilon, tabu)
...
class ConnectTest(algorithm):
    ...
    def run(self):
        accept = True
        random_query_list = self.random_query_list(2/self.delta)
        for i,j in random_query_list:
            if self.query(i,j) == 1:
                count_black_pixels = self.BFS(i,j)
