...
def main(args):
    ...
    if args.algorithm == "adpseq":
        algorithm =  AdaptiveSeq(objective, solution, epsilon=epsilon, args=args)  
    ...
    solution, adptivity = algorithm.run(args)
    
    print(f"algrithm={args.algorithm} n={objective.getN()} m={objective.m} mono={mono} constraint={k} epsilon={epsilon} fval={solution.getf()} evals={objective.getEvals()} adptivity={adptivity}\n")
